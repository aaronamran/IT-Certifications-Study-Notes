...
- Azure virtual networking:
  ...
  - Communication with on-prem resources through 3 mechanisms > Point-to-site VPN (from external computer back into corporate network), site-to-site VPN (on-prem VPN device/gateway to Azure VPN gateway. Devices in Azure appear as being on local network) and Azure ExpressRoute (private connection to Azure that doesn't travel over the internet. Great bandwidth and high levels of security)
...
- Azure Migrate (for large scale migration):
  - Service to migrate from on-prem to cloud. Has the following integrated tools:
    ...
    - Azure Migrate: Server Migration (Migrate VMs, physical servers adn public cloud VMs to Azure)
